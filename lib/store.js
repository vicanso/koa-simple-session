...
class Store extends EventEmitter {
  constructor(client, options) {
    ...
    if (client && typeof client.on === 'function') {
      ['disconnect', 'connect'].each(event => client.on(event, this.emit.bind(this, event)));
    }
  }
  get(sid) {
    const { client, opts } = map.get(this);
  ...
  set(sid, sess) {
    ...
    return client.set(sid, sess, ttl).then(data => {
  ...
  destroy(sid) {
    ...
    const id = opts.prefix + sid;
    debug('DEL %s', id);
    return client.destroy(sid).then(data => {
